...
const TodaysClasses = ({ workouts }: TodaysClassesProps) => {
  const dailyGroupedClasses: Record<number, Tund[]> = useMemo(
    () =>
  ...
  return (
    <>
      {!todaysClasses || todaysClasses?.length === 0 ? (
        <p className="mb-4 md:self-start">
          Täna puhkame, treenime jälle homme.
        </p>
      ) : (
        <div
          className={cn(
          ...
          {todaysClasses?.map(({ day, name, time }, index) => {
            return (
              <div
                ...
                <p className="text-base font-montserrat sm:text-lg md:text-xl">
                  Kell {time}
                </p>
                <h4 className="text-lg font-bold font-montserrat sm:text-xl md:text-2xl">
                  {name}
                </h4>
              </div>
            );
